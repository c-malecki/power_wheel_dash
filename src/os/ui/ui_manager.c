...
static void button_touch_event_cb(lv_event_t *event) {
  if (lv_event_get_code(event) == LV_EVENT_CLICKED) {
    ...
    UI_View_Button_t *button = lv_obj_get_user_data(target);

    OS_Event_t os_event = {0};
    bool post_event = false;

    if (button->action_type == UI_VIEW_ACTION_NAVIGATE) {
      os_event.type = OS_EVENT_NAVIGATE;
      os_event.data.view_id = (UI_View_IDs)button->action_data;
      post_event = true;
    } else if (button->action_type == UI_VIEW_ACTION_SET_VALUE) {
      os_event.type = OS_EVENT_LIGHT_CHANGE;
      os_event.data.color = (Light_Colors)button->action_data;
      post_event = true;
    }
...
lv_obj_t *create_layout(lv_obj_t *cur_screen, const UI_View_t *new_view) {
  ...
  if (new_view->layout == UI_VIEW_LAYOUT_GRID) {
...
lv_obj_t *create_button(lv_obj_t *parent, const UI_View_Button_t *button) {
