...
void Light_Controller_RX(OS_Event_t *os_event) {
  ...
  case OS_EVENT_LIGHT_SELECT: {
    // ESP_LOGI("LIGHT_CONTROLLER", "event received: os_EVENT_LIGHT_SELECT");
    headlight_color = (G_Color_ID)os_event->payload;
    LEDDriver_SetHeadlights((G_Color_ID)os_event->payload);
    break;
  }
